...
-- Function: spcursosdisponiblesasignacionretrasada(integer, integer, integer, integer)
  
-- DROP FUNCTION spcursosdisponiblesasignacionretrasada(integer, integer, integer, integer);
...
begin
 ...
 select distinct cc.curso, cc.codigo, cc.nombre, cc.traslape, cs.seccion, cs.nombre, ee.carnet, (ee.primerNombre || ' ' || ee.segundoNombre || ' ' || ee.primerApellido || ' ' || ee.segundoApellido), eca.asignacion
	...
	join adm_periodo ap on ecla.periodo = ap.periodo and ap.ciclo = _ciclo and ap.estado = 1
	join est_estudiante ee on ee.estudiante = ecla.estudiante and ecla.estudiante = _estudiante
	where ecla.carrera = _carrera and eca.asignacion not in
	(select ar.asignacion from est_asignacionRetrasada ar 
	join est_cur_asignacion eca2 on ar.asignacion = eca2.asignacion and eca2.estado = 1 
	join est_ciclo_asignacion ecla2 on eca2.ciclo_asignacion = ecla2.ciclo_asignacion and ecla2.estudiante = _estudiante 
	join est_pago pa on pa.pago = ar.pago 
	where pa.periodo = _periodo);
end;
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100
  ROWS 1000;
ALTER FUNCTION spcursosdisponiblesasignacionretrasada(integer, integer, integer, integer)
  OWNER TO postgres;
